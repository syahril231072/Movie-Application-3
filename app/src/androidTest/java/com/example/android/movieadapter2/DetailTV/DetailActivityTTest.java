...
public class DetailActivityTTest {
    private Movie dummyCourse = FakeDataDummy1.generateDummyCourses().get(0);

    @Rule
    public ActivityTestRule<DetailActivityT> activityRule = new ActivityTestRule<DetailActivityT>(DetailActivityT.class) {
        @Override
        protected Intent getActivityIntent() {
            Context targetContext = InstrumentationRegistry.getInstrumentation().getTargetContext();
            Intent result = new Intent(targetContext, DetailActivityT.class);
            result.putExtra(DetailActivityT.EXTRA_MOVIE, dummyCourse.getId());
            return result;
        }
    };
    @Before
